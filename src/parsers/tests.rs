...
fn test_format_scan_scope() {
    assert!(format_scan_scope(0, 0, Some("none")).contains("0 packages"));
    assert!(format_scan_scope(10, 0, None).contains("blocklist only"));
    assert!(format_scan_scope(225, 225, Some("local")).contains("OSV=local"));
    assert!(format_scan_scope(5, 5, Some("online")).contains("online api"));
    assert!(!format_scan_scope(225, 225, Some("local")).contains("cap"));
}
